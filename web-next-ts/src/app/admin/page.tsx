...
export default function AdminPage(): JSX.Element {
  ...
  const handleActiveOption = (option: ActiveOptionEnum): void =>
    ...
    const fetchData = async () => {
      try {
        const responseCount = await fetch(
          `http://localhost:14003/api/${activeOption}/count?search=${search}`,
        );
        const resultCount = await responseCount.json();
        setCount(resultCount.data); // Ensure this matches your expected structure

        const responseData = await fetch(
          `http://localhost:14003/api/${activeOption}?search=${search}&limit=${limit}&page=${page}`,
        );
